    ...
    for (let i = 1; i <= count; i++) {
        wrap.innerHTML += `
            <div class="bg-black/30 border border-white/10 rounded-xl p-4 space-y-3">
                <p class="text-sm font-semibold">Guest ${i} — required info</p>
                <div class="grid sm:grid-cols-2 gap-3">
                    ...
                    <div>
                        ...
                        <input data-guest="${i}" data-field="email" type="email" required placeholder="name@email.com"
                               class="guest-field w-full bg-gymmy-dark border border-white/20 rounded-xl px-3 py-2 text-sm">
                    </div>
                    <div>
                        <label class="block text-xs text-gray-400 mb-1">Date of birth</label>
                        <input data-guest="${i}" data-field="dob" type="date" required
                               class="guest-field w-full bg-gymmy-dark border border-white/20 rounded-xl px-3 py-2 text-sm">
                        <p class="text-[11px] text-gray-500 mt-1">Use the calendar to enter their birthday.</p>
                    </div>
                </div>
            </div>`;
    }
...
function memberId(username) {
    return encodeURIComponent(username).replace(/[^a-zA-Z0-9]/g, '_');
}
...
async function renderAccountsList() {
    ...
    list.innerHTML = data.users.map(u => {
        const id = memberId(u.username);
        const leftClass = u.peakHoursLeft <= 3 ? 'text-red-400' : 'text-gymmy-green';
        const upcoming = (u.bookings || []).filter(b => b.date >= formatDate(new Date())).slice(0, 5);
        return `
        <div class="bg-black/30 border border-white/10 rounded-xl overflow-hidden">
            <button type="button" class="w-full text-left px-4 py-3 flex justify-between items-center gap-3" onclick="document.getElementById('member-edit-${id}').classList.toggle('hidden')">
                <div>
                ...
                <div class="text-right shrink-0">
                    <p class="${leftClass} font-bold">${u.peakHoursLeft}h left</p>
                    <p class="text-[11px] text-gray-500">${u.peakHoursUsed} used / ${u.peakHoursMax} max</p>
                </div>
            </button>
            <div id="member-edit-${id}" class="hidden border-t border-white/10 px-4 py-4 space-y-3">
                <p class="text-xs text-gray-500">Base 21 peak hours ${u.peakAdjust ? (u.peakAdjust > 0 ? '+ ' + u.peakAdjust + ' extra' : '− ' + Math.abs(u.peakAdjust) + ' removed') : '(no adjustment)'} this month.</p>
                <div class="grid sm:grid-cols-2 gap-2">
                    <input id="adm-email-${id}" type="email" value="${escapeHtml(u.email || '')}" placeholder="Email"
                           class="w-full bg-gymmy-dark border border-white/20 rounded-xl px-3 py-2 text-sm">
                    <input id="adm-phone-${id}" type="tel" value="${escapeHtml(u.phone || '')}" placeholder="Phone"
                           class="w-full bg-gymmy-dark border border-white/20 rounded-xl px-3 py-2 text-sm">
                </div>
                <textarea id="adm-address-${id}" rows="2" placeholder="Address"
                          class="w-full bg-gymmy-dark border border-white/20 rounded-xl px-3 py-2 text-sm resize-none">${escapeHtml(u.address || '')}</textarea>
                <textarea id="adm-notes-${id}" rows="2" placeholder="Admin notes (only you see this)"
                          class="w-full bg-gymmy-dark border border-white/20 rounded-xl px-3 py-2 text-sm resize-none">${escapeHtml(u.notes || '')}</textarea>
                <input id="adm-pass-${id}" type="text" placeholder="New password (leave blank to keep)"
                       class="w-full bg-gymmy-dark border border-white/20 rounded-xl px-3 py-2 text-sm">
                <div class="flex flex-wrap gap-2 items-center">
                    <button type="button" onclick="adjustMemberHours('${escapeHtml(u.username)}', 1)" class="px-3 py-2 rounded-xl border border-gymmy-green/40 text-gymmy-green text-sm">+1 peak hour</button>
                    <button type="button" onclick="adjustMemberHours('${escapeHtml(u.username)}', -1)" class="px-3 py-2 rounded-xl border border-red-400/40 text-red-400 text-sm">−1 peak hour</button>
                    <button type="button" onclick="adjustMemberHours('${escapeHtml(u.username)}', 5)" class="px-3 py-2 rounded-xl border border-white/20 text-sm">+5</button>
                    <button type="button" onclick="adjustMemberHours('${escapeHtml(u.username)}', -5)" class="px-3 py-2 rounded-xl border border-white/20 text-sm">−5</button>
                </div>
                <button type="button" onclick="saveMemberAdmin('${escapeHtml(u.username)}')" class="w-full bg-gymmy-gold text-black font-bold py-2 rounded-xl text-sm">Save profile</button>
                <p id="adm-msg-${id}" class="hidden text-xs"></p>
                <div>
                    <p class="text-xs text-gray-400 mb-1">Upcoming bookings</p>
                    ${upcoming.length ? upcoming.map(b => `<p class="text-xs text-gray-300">${b.date} · ${formatRange(b.start, b.duration)} · Bay ${b.bay}${b.guestCount ? ' · ' + b.guestCount + ' guest(s)' : ''}</p>`).join('') : '<p class="text-xs text-gray-500">None upcoming</p>'}
                </div>
            </div>
        </div>`;
    }).join('');
...
async function saveMemberAdmin(username) {
    const id = memberId(username);
    const msg = document.getElementById('adm-msg-' + id);
    try {
        const body = {
            email: document.getElementById('adm-email-' + id).value.trim(),
            phone: document.getElementById('adm-phone-' + id).value.trim(),
            address: document.getElementById('adm-address-' + id).value.trim(),
            notes: document.getElementById('adm-notes-' + id).value.trim()
        };
        const pass = document.getElementById('adm-pass-' + id).value;
        if (pass) body.password = pass;
        await api('/api/accounts/' + encodeURIComponent(username), { method: 'PUT', body: JSON.stringify(body) });
        msg.textContent = 'Saved.';
        msg.className = 'text-xs text-gymmy-green';
        msg.classList.remove('hidden');
        await renderAccountsList();
        document.getElementById('member-edit-' + id)?.classList.remove('hidden');
    } catch (ex) {
        msg.textContent = ex.message;
...
async function adjustMemberHours(username, delta) {
    try {
        await api('/api/accounts/' + encodeURIComponent(username), {
            method: 'PUT',
            body: JSON.stringify({ peakDelta: delta })
        });
        await renderAccountsList();
        const id = memberId(username);
        document.getElementById('member-edit-' + id)?.classList.remove('hidden');
