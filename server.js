...
function publicUser(u) {
  return {
    ...
    role: u.role,
    notes: u.notes || '',
    peakAdjust: Number(u.peakAdjust || 0)
  };
}

function peakAllowance(user) {
  return MAX_PEAK_HOURS + Number((user && user.peakAdjust) || 0);
}

function peakSnapshot(store, user) {
  const used = peakUsedThisMonth(store, user.username);
  const max = peakAllowance(user);
  return {
    peakHoursUsed: used,
    peakHoursMax: max,
    peakHoursLeft: Math.max(0, max - used),
    peakAdjust: Number(user.peakAdjust || 0),
    peakBase: MAX_PEAK_HOURS
  };
}
...
app.get('/api/me', (req, res) => {
  ...
  res.json({
    ...
    ...peakSnapshot(ctx.store, ctx.user)
  });
});
...
app.get('/api/accounts', (req, res) => {
  const ctx = requireAdmin(req, res);
  if (!ctx) return;
  const users = ctx.store.users.map(u => {
    const snap = peakSnapshot(ctx.store, u);
    const bookings = ctx.store.bookings
      .filter(b => b.username === u.username)
      .sort((a, b) => b.date.localeCompare(a.date) || b.start - a.start);
    return { ...publicUser(u), ...snap, bookings };
  });
  res.json({ users });
});
...
app.put('/api/accounts/:username', (req, res) => {
  ...
  const user = ctx.store.users.find(u => u.username.toLowerCase() === username.toLowerCase());
  if (!user) return res.status(404).json({ error: 'Member not found.' });

  if (req.body.email !== undefined) user.email = String(req.body.email || '').trim();
  ...
  if (req.body.peakAdjust !== undefined && req.body.peakAdjust !== '') {
    user.peakAdjust = Number(req.body.peakAdjust);
  }
  if (req.body.peakDelta !== undefined && req.body.peakDelta !== '') {
    user.peakAdjust = Number(user.peakAdjust || 0) + Number(req.body.peakDelta);
  }
  if (!Number.isFinite(user.peakAdjust)) user.peakAdjust = 0;

  saveStore(ctx.store);
  res.json({ user: { ...publicUser(user), ...peakSnapshot(ctx.store, user) } });
});
...
app.post('/api/bookings', async (req, res) => {
  ...
  const snap = peakSnapshot(ctx.store, ctx.user);
  if (peakHours > 0 && snap.peakHoursUsed + peakHours > snap.peakHoursMax) {
    return res.status(400).json({
      error: `Not enough peak hours. You only have ${snap.peakHoursLeft} left this month.`
    });
  }
  ...
  res.json({ booking, peakHoursLeft: Math.max(0, peakSnapshot(ctx.store, ctx.user).peakHoursLeft) });
});
